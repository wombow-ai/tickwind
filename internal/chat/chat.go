...
var widgetTypes = []string{
	"valuation_table", "fundamentals_table", "kline", "indicators", "indicator_history", "seasonality", "relative_strength", "earnings_reaction",
...
func toolSpecs(lang string, general, hasUserData, hasWeb bool) []enrich.ChatTool {
	...
	tools = append(tools,
		...
		enrich.ChatTool{
			Name:        "surface_widget",
			Description: d("内联渲染一个真实 Tickwind 图表/表格(用户看到真实控件)。优先用它\"展示\"数据而非罗列数字(基本面问题→fundamentals_table,估值→valuation_table,价格/技术面→kline,某个技术指标的历史走势→indicator_history 并指定 indicator;某股票的月度季节性/历史规律→seasonality;某股票相对大盘(SPY)的强弱/跑赢跑输→relative_strength;某股票历次财报前后的历史波动→earnings_reaction)。但若事实工具说该股票没有公司基本面(例如 ETF),就不要 surface fundamentals_table/valuation_table。只会收到确认,不会拿到数据 —— 正常。", "Render a real Tickwind chart/table inline (the user sees the actual widget). PREFER showing a widget over reciting many numbers (fundamentals_table for a fundamentals question, valuation_table for valuation, kline for price/technicals, indicator_history WITH an indicator for one technical indicator's time series, seasonality for a stock's month-of-year historical return pattern, relative_strength for how a stock has performed vs the S&P 500 / market, earnings_reaction for how a stock has historically moved around its earnings) — but if a fact tool says the stock has no company fundamentals (e.g. an ETF), do NOT surface fundamentals_table/valuation_table. You only get a confirmation back, not the data — that's expected."),
			Parameters: map[string]any{"type": "object", "properties": map[string]any{
