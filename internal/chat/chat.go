...
func systemPrompt(ticker, lang, material string, general, hasUserData, hasWeb bool, mode string) string {
	...
	b.WriteString(d("数字两种。(a) 当前或历史的事实数字 —— 价格、市盈率、营收、EPS、RSI、百分位、持仓、公司披露的日期 —— 必须逐字来自工具结果或 <facts> 块,绝不臆造、估算或重算。也不要凭记忆引用外部基准(如\"大盘约 18 倍\"\"同业平均 25 倍\")—— 要谈相对大盘/同业,就用工具的 relative/百分位数据(它们是 grounded 的)。来自网络的事实是带出处的背景:注明来源即可引用(\"据 GraniteShares 申报,约 7 月 2 日生效 [来源]\"),但不当作 Tickwind 核实过的数字、也不据此另算新数。(b) 目标价、预测、展望、买卖判断是你自己的看法 —— 可以给,讲清是【你的分析】,基于已取到的数字。\n\n",
		"Two kinds of number. (a) A CURRENT or HISTORICAL factual figure — price, P/E, revenue, EPS, RSI, a percentile, a holding, a date the company reported — MUST come verbatim from a tool result or the <facts> block; never invent, estimate, or recompute one. Don't cite an external benchmark from memory either (\"the market trades near 18x\", \"peers average 25x\") — to compare against the market/peers, use the tools' relative / percentile data (those are grounded). A fact found on the web is attributed background — state it WITH its source (\"per GraniteShares' filing, effective ~Jul 2 [source]\"), but never as a Tickwind-verified number, and never derive a new calc from it. (b) A target, forecast, outlook, or buy/sell call is YOUR view — give it, framed as YOUR analysis, built on the figures you pulled.\n\n"))
