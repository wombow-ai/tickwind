...
- **2026-06-28 — 🎁 Evergreen AAPL DEMO deep report — free, anon-viewable, quota-free (measurement/proof/reliability roadmap #2; be `cee50d6` + fe `<this commit>`; SHIPPED).** The biggest free→Pro objection is "can't see the $99 report before paying." Now ONE mega-cap (**AAPL**) deep report is fully unlocked to EVERYONE (incl. anon prospects) — the highest-converting honest sales asset + a top SEO page; every other ticker stays gated/paywalled exactly as before. **Backend (be `cee50d6`):** `demoReportTickers={"AAPL"}` + `isDemoReport()`; in `getResearchDeep` a demo ticker bypasses the login gate (anon can view), skips the per-user monthly quota (CHECK skipped via `if !demo`, CHARGE guarded by `userID!=""`), and `serveDeepReady(…, ticker)` skips truncation for it. Bounded by the existing global daily cap + per-month cache + single-flight + lang clamp (≤2 quota-free gens/month). **FE (fe `<this commit>`):** `config.ts` `isDemoReportTicker`/`DEMO_REPORT_PATH` (mirrors backend); `DeepResearchView` fetches for anon when the ticker is demo (others stay login-gated); `/pro` page + the locked-report `PaywallBanner` link to "See a full example (AAPL)"; the `/stock/[ticker]/research` page is **`index:true` for a demo ticker** (SEO showcase) with accurate "free complete example" copy (others stay noindex). **Reviewed by a 2-lens opus adversarial pass (scope-leak + abuse/cost): SHIP IT, no blockers** — exemption is exactly {AAPL} (canonicalized, no spoof), cache never mutated, quota-free path bounded. Tests: AAPL gated-behavior tests switched to non-demo tickers (MSFT/NVDA); added `TestGetResearch_DemoReportAnonQuotaFree`. Gate Go gofmt/build/vet + web tsc/build green. **NEXT roadmap: ③React error boundaries, ④/stock internal links.** ⚠️ owner: an anon visitor at `/en/stock/AAPL/research` sees the full report (no login wall); other tickers still gate.
- **2026-06-28 — 📊 Conversion-funnel instrumentation (owner picked the "measurement + proof + reliability" roadmap; #1 of 4; be `c1e8548` + fe `<this commit>`; SHIPPED).** Context: an opportunity-scan Workflow (5 dims) found the product has saturated DEPTH — the next leverage is **measurement + proof + reliability + connecting existing assets**, NOT more features. Owner chose that path; this is the foundation (#1): the product takes real money but had **zero funnel visibility**. **Backend (be `c1e8548`):** `store.FunnelEvent`/`FunnelStat` + `SaveFunnelEvent`/`FunnelSummary` (memory + postgres `funnel_events` table + split → durable Market store); **`POST /v1/event`** — public, fire-and-forget (204), CLOSED event/surface enum (unknown dropped, never stored), 1KB cap, per-IP rate-limited, attaches user+`tierOf` when a bearer is present (anon otherwise); **`GET /v1/admin/funnel?days=N`** — admin-gated (event,surface) aggregate; the Stripe webhook fires `subscription_active` on a checkout that lands active/trialing. **FE (fe `<this commit>`):** `trackEvent(event,surface,token?)` helper (fetch keepalive, never throws) fires `paywall_view` from all 5 walls (DeepResearchView/deep_research, SignalsCard/indicators, SignalsScreen/screener, BacktestWidget/backtest, ChatHub/chat-low-quota), `pro_view` on /pro mount, `checkout_started` on Subscribe; a minimal **admin page `/admin/funnel`** (stages paywall→/pro→checkout→subscribed + per-surface breakdown + conversion %; 403→"Admin only", not nav-linked). Funnel = `paywall_view → pro_view → checkout_started → subscription_active`. **Reviewed by a 2-lens opus adversarial pass: SHIP IT, no blockers** (closed-enum + cap + rate-limit bound the public write; PII = Supabase uuid only). Gate: Go gofmt/build/vet/`-race` + web tsc/build green. **DEFERRED follow-up:** a `funnel_events` retention pruner (~180d) — task chip filed; slow time-growth, not urgent. **NEXT in the roadmap: ②open an AAPL demo report, ③React error boundaries, ④/stock internal links** (all non-owner-gated). ⚠️ owner: visit `/admin/funnel` (logged in as admin allalphaplus@gmail.com) to see the funnel once events accrue.
- **2026-06-28 — 🧵 AI Chat: persisted step groups COLLAPSE to one line (expandable), in position (owner; fe `<this commit>`; FRONTEND-only; SHIPPED).** Owner: on a reloaded conversation the interleaved step groups rendered FULLY EXPANDED (every tool row shown) — wanted each same-stage group merged to one line with an expand button, like Claude (the collapsed "正在梳理… ›" summary). New `TraceGroup` in `chatRender.tsx`: a multi-step group renders a one-line `› ✓ {n} steps` toggle (default collapsed) that expands to the full chain; a single-step group shows its row as-is; both stay IN POSITION (interleaved with the narration). Re-added the `ChevronDown/ChevronRight` imports + a `chat.stepsN` i18n key (en "{n} steps" / zh "{n} 个步骤"). The LIVE streaming view is unchanged (steps stream in expanded as it works; they collapse to the tidy groups on done/reload — the Claude pattern). Gate: web tsc/build green. Preview-verified: narration → `› ✓ 4 steps` → narration → `› ✓ 3 steps` → `✓ Checking recent news` (single) → answer; clicking "4 steps" expands to the 4 rows (aria-expanded→true). ⚠️ owner visual: reload a tool-using turn → each multi-step group is one collapsed line with an expand chevron, in place.
- **2026-06-28 — 🧵 AI Chat: answer no longer truncates + the interleaved chain now PERSISTS on reload (owner 2 items; be `35b6b56` + fe `<this commit>`; SHIPPED, deployed, reviewed).** **#1 truncation:** owner saw a DRAM "deep analysis" answer cut off mid-sentence at "风险提示:本分析基于当". Root cause: `enrich.chatMaxTokens=1500` — the now-longer full-advisor answers (bull/bear + action tiers + targets + risk note) exceeded it → completion truncated. Bumped to **2800** (bounded by the per-thread token cap + per-user meter). **#2 persist interleave:** owner: during streaming the chain is interleaved (good), but on done/reload all steps "jumped back to the top" (the collapsed "Steps · N" trace was prepended; the interleave was lost). Key realization: in the ReAct loop ALL steps precede the final answer, so the interleave only exists via the model's PREAMBLE narration between step groups — which wasn't persisted. **`AnswerStream` now builds an ordered `pre []Block`:** each iteration's preamble content → `{kind:"narration",text}`, then that iteration's steps → `{kind:"trace",steps:group}`; `finishStream` prepends it → stored blocks = `[narration][trace][narration][trace]…[text][widgets]` in order. **Safety preserved:** `assistantProse` still joins ONLY `kind=="text"` → narration + trace never re-enter the model (TestAssistantProseSkipsTrace updated to assert narration skipped; TestAnswerStreamInterleaved locks the ordering). **FE (`chatRender.tsx`):** `BlockView` renders `narration`→prose, `trace`→inline `ExecChain` rows IN POSITION (removed the collapsed `TraceBlock` toggle + the orphaned `chat.trace` i18n key); `ChatBlock.kind` += `narration`. Live streaming (liveItems) unchanged → live↔reloaded now match. **Reviewed by a 2-lens opus adversarial Workflow: SHIP IT, no blockers/highs** (safety verified end-to-end; backward-compat with old `[trace][text]` messages holds); its 3 cosmetic LOWs folded in. Gate: Go gofmt/build/vet/`-race` + web tsc/build green; preview-verified (done-state shows narration → ✓steps → narration → ✓step → answer, steps in position). ⚠️ owner visual: real Pro /chat — a long "deep analysis" answer finishes (no mid-sentence cut); reload a tool-using turn → the steps stay interleaved in place, not collapsed at the top.
- **2026-06-28 — 🧵 AI Chat: INTERLEAVED Claude-style live chain (step→narration→step→answer) + DRAM ETF "no filing" fix (owner 2 items; be `78bc991` + fe `<this commit>`; SHIPPED, deployed, verified).** **#1 (chat chain UX):** owner wanted the Claude-style interleaved ReAct view (a tool action, then its narration, … then the final answer) instead of the old TOP-STACKED chain (all gray steps above the answer). Was NOT implemented (top-stacked). **Key enabler:** `AnswerStream` already streams content tokens + steps in chronological order across ReAct iterations (`onToken`/`onStep` interleaved) — only the FE flattened them. **FE rewrite:** `ChatThreadPanel` now builds an ordered `liveItems` list (`{kind:'text'|'step'}`) as tokens/steps arrive (a token extends the last text item; a step pushes a step item); `chatRender.MsgRow` renders that interleaved during streaming (extracted `ExecStepRow` reused for one row; the last item is the live one — a step pulses, a text chunk carries the streaming caret). On `done` it reconciles to the persisted blocks (collapsed "Steps · N" trace + final answer + widgets); on reload the collapsed trace (intermediate narration isn't stored — acceptable, matches Claude collapsing past turns). Preview-verified (mock data): narration → ✓step → narration → ✓step → streaming-answer-with-caret; pulsing globe for a running tool; collapsed "Steps · 2" on done. **#2 (DRAM ETF holdings):** the detail page showed no holdings AND the endpoint **502'd**. Root cause: DRAM (+ RAM/DRAL, all brand-new Roundhill ETFs) aren't in SEC's ticker directory yet → `edgar.lookup` returns "ticker not found" → `getETFHoldings` treated it as a gateway 502. Added a typed `edgar.ErrTickerNotFound`; the handler now treats it (+ `ErrNoNPORT`) as "no filing" and, for a KNOWN ETF (per symbols), returns a clean **200 `{holdings:[], no_filing:true}`** → the panel shows a brief "holdings appear after the fund's first SEC filing" note (`etf.noFiling` i18n) instead of erroring/hiding. **Owner decided (AskUserQuestion): the note, NOT a web-sourced-holdings fallback** (no paid web search on public pSEO pages; DRAM genuinely has no free structured source — only the web has its holdings, which is why the chat web-searches them). Gate: Go gofmt/build/vet/`-race` + web tsc/build green. **LIVE-verified:** DRAM endpoint `200 no_filing:true` (was 502), QQQ still 200. ⚠️ owner visual: real Pro /chat — ask something that uses tools → the steps + narration now interleave (action → text → action → answer); /stock/DRAM → a "holdings after first SEC filing" note (no error).
- **2026-06-28 — 🔎 AI Chat: MINIMAL "resourceful" prompt — never dead-end on "not in our data" without searching + deleted the Auto/Focused/Explore toggle (owner-reported; be `27c3fda` + fe `<this commit>`; SHIPPED + LIVE-verified).** Owner: even after the full-advisor change the chat still sometimes said "I don't have these in Tickwind's database… go check the issuer yourself" WITHOUT trying the web (e.g. "when do the US 2x SK-Hynix ETFs SKUU/SKDD launch?"), while free Claude answered it richly. Owner: drastically simplify the prompt ("let the AI free-play") + delete the toggle (no differentiation). **Root-caused: Haiku's tool-use was NON-DETERMINISTIC** — the SAME question (current deployed code, web_search=true) sometimes searched 3× and answered well, sometimes called ZERO tools and refused (the owner's run, msg at 10:07Z had no trace block = no tools). Causes: (1) the `search_web` tool Description said "ONLY for background — never restate as fact" (discouraged using it for a factual "when does X launch" question); (2) a long prompt with grounding tensions let the model rationalize "this is a fact I can't ground → just say I don't have it"; (3) no forceful "never refuse without searching" rule. **Designed via a 3-candidate opus Workflow → synthesize → max-effort adversarial review** (the review caught a real BLOCKER: the web-result envelope in `chatwebsearch.go` + `get_news_context` still said "never quote a number" — contradicting the new prompt, making Haiku hedge on the SKUU date even AFTER searching). **Shipped (backend `27c3fda`):** `systemPrompt()` rewritten to a SHORT, action-first prompt — **RULE ZERO leads** ("be resourceful: pull facts / `get_etf_holdings` / `search_web` for anything not already in hand; NEVER say 'I don't know / not in our data / check it yourself' without first trying a tool; a pure definition needs no search; for the anchored stock the `<facts>` block already has its core figures"). Kept compact: number-discipline (facts from tools; a web fact is quoted WITH its source, not as a Tickwind figure; targets/forecasts are the model's view), injection defense, ETF stay-on-subject, privacy, one-line widget/style. **Dropped** the few-shot + GROUNDING/STYLE/FORMATTING blocks + the explore/focused mode appendices (`mode` now INERT, param kept). **Broadened the `search_web` Description** (permissive: news, launch/IPO dates, niche funds we don't cover, any factual question not in our data). **BLOCKER fix:** relaxed the `chatwebsearch.go` envelope header + `get_news_context` to "quote a fact WITH its source, not as a Tickwind-verified figure, never derive a new number" (consistent with the prompt). **Toggle deleted (fe `<this commit>`):** removed the Auto/Focused/Explore segmented control + `tw-chat-mode` localStorage from `ChatThreadPanel`, `modeBody` + the `mode` params from `api.ts`, the `chat.mode.*` dict keys. Anti-hallucination on facts preserved (structural + the number rule); injection envelope unchanged; deep report untouched. `TestSystemPromptModes` rewritten (mode inert; RULE ZERO present + hasWeb-gated). Gate: Go gofmt/build/vet/`-race` + web tsc/build green. **LIVE-verified via a fresh-quota-per-turn E2E** (the first run's empties were free-weekly-token exhaustion, not search failures — re-measured cleanly): SKUU x6 → **searched 6/6, 0 genuine refusals** (every run engaged via web search with sources, no "go check it yourself" dead-end); AAPL "P/E?" → grounded "**34.2x** as of Q2 FY2026" from the pre-loaded `<facts>` (no fabrication); DRAM "holdings?" → tried `get_etf_holdings` first → web fallback → SK hynix/Samsung/SanDisk/WD; NVDA "latest news" → searched + attributed ("$39.3B [per NVIDIA Newsroom]") quoting a web number WITH its source (the BLOCKER fix live) + no instruction-following. ⚠️ owner visual: real Pro /chat — ask "when do SKUU/SKDD launch?" → it now searches + answers with sources (no more "go check the issuer"); the mode toggle is gone. **Follow-up (be `d29aa99`, LIVE):** a behavioral spot-check (fresh-user-per-turn) confirmed definitions/chit-chat call NO tools (no over-search), full-advisor + grounding + injection-resist all hold — and surfaced ONE residual softening: the minimal rewrite had dropped the "no external-benchmark-from-memory" clause, so the model quoted "~18-19x market average" from memory. Re-added a one-phrase guard (don't cite a market/peer average from memory → use the tools' relative/percentile data). Re-verified: it now pulls + surfaces the grounded scorecard percentile for market comparisons (residual hedged "~18-20x historical median" as color is acceptable; company-specific facts stay tool-grounded).
