...
- 🏗 **Alerts** (greenlit, multi-tick): ✅ data layer (`store.Alert` {ticker/kind/threshold/active}
  ...
  → MarkTriggered; live, log-verified). Next: ⬜ ④c frontend "triggered" badge (+ TS `triggered_at`)
  + pct_move eval (Quote.PrevClose); ⬜ ④d new_filing eval → ⬜ ⑤ web-push (self-gen VAPID).
