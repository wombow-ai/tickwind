...
## 🎨 v8 UX 修(owner 2026-06-14 截图反馈)
> **✅①首页榜单行高对齐**:Boards 行(Hot/Opportunity/Guru-watch)默认 items-stretch 致 Hot/Opportunity 被 Guru 内容拉高、下方留空。改 `items-start`——各卡自然高度顶对齐(实测 Hot/Opp 225px、Guru 286px,不再被撑到 286 留白)。预览 3 列实测干净。
> **✅②分享卡语言+画质**:(a)语言:OG 卡引擎硬编码中文 chrome(badge"中文美股数据台"+footer tag)致英文 UI 下卡片仍中文。加 `lang` 参数贯穿 og.ts→/api/og 路由(badge/tag 随 lang:en"Data-first US stocks"/"Congress·13F·Options flow·Insider buys")→ShareCardButton 自动注入当前 UI lang;各 caller 卡文案改语言感知(SentimentChip[首页/被点名]、StockView'数据延迟'、FollowTradeSim 客户端跟 useLang;fund/indicator 服务端默认英文)。(b)画质:卡片 1200×630 设计经 transform scale(2) 渲染到 2400×1260(高清,小红书/高 DPI 转发更锐),ogImageMeta 同步 2400×1260。**预览实测渲染**:英文卡全英文 chrome+锐利、中文卡 CJK 正确,2x 无裁切。lint 0/build 绿。

## 🎛️ v8 个股可选指标(owner 2026-06-14:R1 落地为个股可用工具)
> owner 选 Phase A+B。诚实上限~138 可算(非282;sentiment 98=期权/链上/宽度/宏观源未接,技术 patterns/盘中 VWAP/IV、基本面 forecast/headcount/index-beta/长历史 不可算)。设计 docs/research/2026-06-14-selectable-indicators-design.md。
> **✅ Phase A(前端 picker)+ Phase B 增量1(后端扩算)本 commit**:**Phase B**:`internal/indicators` 加 `technical_more.go`(64 技术指标 closures,纯 OHLCV:WMA/DEMA/TEMA/HMA/KAMA/ALMA/VIDYA/KC/Donchian/Supertrend/SAR/DMI-ADX/Vortex/Aroon/CCI/Williams%R/TSI/TRIX/PPO/StochRSI/UO/Fisher/OBV/ADL/CMF/MFI/Klinger… + ~30 纯 helper)+ `fundamental_more.go`(26 Group-0 比率,无新 XBRL:市值/EPS/BVPS/SPS/PS/D-E/ROA/资产周转/OCF比/FCF转化/派息率/Tobin-Q/PE-LYR…)。`compute.go` 合并 4 子注册表(共 **109 closures**,disjoint 测试守)+ `computedIDs()` 改为**按 registry 迭代**(未实现 id 直接 absent 不出"未计算"行)。**对抗式复核判定 faithful 无 must-fix**:每个 Value 仅 setOK 真值、缺输入/零分母/不足→setInsufficient 无值,`TestComputedClosuresNeverPanicOrFabricate` 跑遍每个 closure 空输入断言不杜撰。**Phase A**:`IndicatorsPanel` 加选择模型(默认=payload 的 P0 集=今日面板无回归,随后端自动长大)+ `IndicatorPicker`(搜索/domain 筛/勾选增删/上下排序/重置/a11y)+ `indicatorSelection.ts`(localStorage `tickwind.indicators.v1`,server-prefs 可后接)+ i18n。前端实测 picker 工作。go build/vet/gofmt/test 全绿;web lint 0/build 绿。**后续增量**:Group1-4 新 XBRL 比率(→~128)、Group5 Altman-Z/Piotroski、登录云端 /v1/me/prefs 同步。

## 🛠️ v8 owner 反馈批(2026-06-14 · 4 项)
> **①审查指标正确性(2 个对抗式 subagent,owner 要求)= 双双判定 FAITHFUL 零 bug**:技术指标 audit 独立复算 11 个到 10 位小数吻合(RSI 63.49/CCI 92.16/ADX 32.08/MFI 64.85/ATR 2.96…),Wilder 平滑/权重/符号全对、无杜撰;基本面 audit 手算全部 36 比率(PE 10x/PS 8x/ROA 13.33%/SGR 35%…)、单位正确、无杜撰、近似项均文档化。结论:109 指标计算正确。
...
- **✅④Opportunity board 变空修复**(Go,本批):根因=启动时 6 次 bulk Alpaca snapshot 连发触发 **429**,`recompute()` 在 price-fetch 错误时置空 prices → 所有行被 price≤0 gate 掉 → **把好的 23 行板子覆盖成空**(同 audit 的 symbols-refresh 部分失败覆盖 class)。修=price-fetch 错误时保留 last-good 板子不覆盖(genuine 零买入仍诚实置空)+ 防御性 empty-price guard + 回归测试。**follow-up**:①debounce 启动 recompute 连发(429 的根源,**未做**)②~~dei 股本覆盖缺口~~ **✅已修(2026-06-15,commit e864dce)**:`refreshShares` 对 dei 帧未解析的 CIK 兜底 `us-gaap:CommonStockSharesOutstanding`(同样3季帧,dei 仍权威不被覆盖),帧层守 450天 staleness + 0/1股垃圾值→放大板子而不引入错市值(insufficient-not-wrong);cap 带/MinBuyValue/排序/keep-last-good 不动;启动 sweep 即生效。**✅LIVE验证:板子 4→13 行,全部市值在带内($334M–$1.68B),启动日志 `refreshed shares ciks=5796 via_fallback=216`=216 个 dei 缺失 CIK 由 us-gaap 兜底解析。**
...
- **✅增量1:深度 compose harness**(Go,本批):`enrich.ComposeDeepReport`(Fable5 分层 prompt:research_standards/citation_discipline/hedging/scope_boundaries/format_rules/anti_fabrication/self_check + exec overview,zh+en)over 现有 R2 FactSheet;模型 `cfg.LLMDeepModel`(env `LLM_DEEP_MODEL`)默认 fallback 现 `LLMModel`=**零成本/行为变化直到 owner 设贵模型**;更长 token(6000);API `GET /v1/stocks/{t}/research?depth=deep`(独立 `|deep` cache key 不撞 normal;LLM off→data-only 不 503;cap+single-flight)。**反幻觉=结构性安全**:复用 buildMaterial(只格式化字符串无 Raw)+ parseSectionProse(丢弃 stray numeric key)→ 即使 LLM 乱填也注不进错数;`TestComposeDeepReportNeverMutatesNumbers` 断言 facts byte-identical。normal /research 不变。Go gate+ -race 全绿。**✅增量2:登录+全站每用户每天1次生成配额**(Go,本批):仅 depth=deep 门控(normal /research 不变公开)。匿名 deep→401;全局 `(ticker,day,lang,deep)` 缓存命中=任何登录用户**免费**看(不耗配额不调 LLM);缓存 miss+将生成(wantProse=LLM 开+未超全局 cap)时查 `GetDeepQuotaUsed(user,ET-day)`,超 `DeepResearchDailyLimit`(env,默认1)→429;**配额只在成功生成(hasProse)后 IncrDeepQuotaUsed**(data-only/LLM 关不消耗,镜像 ⑤)。store 加 GetDeepQuotaUsed/IncrDeepQuotaUsed(User 库,postgres `deep_research_quota` 表 upsert-increment,启动自动建表,split 路由 User)。fail-open(配额读错=放行不锁死用户)。测试:匿名401/配额429/缓存免费/异用户独立槽/data-only不消耗+ -race。**✅增量3:固定排版报告页+AI Digest 右上角入口按钮**(web,本批):AISummaryCard 右上角 `DeepEntry` 按钮(双语,两 header 态)→ `/stock/{t}/research`。新路由 `app/[locale]/(main)/stock/[ticker]/research/page.tsx`(server shell:per-locale metadata+面包屑 JSON-LD+**noindex,follow**+canonical/hreflang+generateStaticParams 热门子集×2+ISR;**不在 build/server 跑 gated fetch**)。`DeepResearchView`(client)复用 ResearchReport 的 FactCell/BullBearList/CitationChip(导出,公开 R2 tab 不动)=exec overview+各节(Go-fact 表+prose+bull/bear+citations+价格节 KLineChart)。**auth/quota UX 状态机**:匿名→登录解锁 card+公开 /research preview teaser(也服务拉新门控);登录→`getDeepResearch(ticker,token,lang)` 带 Bearer fetch+10-60s loading 态;429→友好"每日1次"提示;llm:false→data-only 优雅;401→回匿名 gate。citations 跨页 rebase 到 `/stock/{t}#anchor`(F3)。22 个 deep.* i18n(en+zh)。tsc 0/build 874 页/eslint 0。**→ AI 深度研报功能性完成(增量1-3:harness+登录配额+视图;免费层=每用户每天1篇)。** **待做仅 ④付费 paywall**(待 owner Yahoo 取舍 docs/owner-confirm.md;贵模型届时经 LLM_DEEP_MODEL 启用)。**flagged follow-up**:同步 deep 调用 10-60s,若生产太慢→async 生成/轮询(未做);~~LLM compose 超时~~ **✅已修(2026-06-15,commit 88eb75c)**:每次 compose 在 handler 边界 `context.WithTimeout`(25s 普通/60s 深度),enrich 用 `NewRequestWithContext` 故超时取消真实出站 HTTP→慢/限流时**快降 data-only** 不再挂到 ~90s;4 处退款路径全在超时触发、深度配额只在真 prose 计、material-events 超时回退 facts-only 不 404、summary 超时降 200 空摘要(真错仍 502);反幻觉测试 byte-identical。**LIVE 验证:uncached SOFI/research data-only 200 @27s(=25s 界+开销),vs 旧 ~90s**。**新发现(已记 CLAUDE.md + 记忆 tickwind-cold-research-3s-reset + owner-confirm #5)**:冷门股研报**首请求间歇性 ~3.0s 空响应**=Cloudflare Tunnel 那一跳(非代码 bug、非本次回归;重试即成),拟前端 retry-once 缓解(下一轮)。

### C. 拉新:核心功能登录门控
- 核心(如指标)未登录只看部分,完整需登录;未登录"部分视图"样式我设计(诚实预览 + 引导登录,不打扰式,守 data-first)。
- **✅已做(2026-06-15,commit 940555d,web)**:个股 `IndicatorsPanel` 未登录显示**前 5 项指标** + 柔和渐隐 + "登录查看全部 N 项"CTA(`LocalLink` 带 locale 前缀);登录用户不变(全量 + customize picker)。**纯 client-side 视图层**(面板本就 client fetch→**零 SEO 影响**);**公开 pSEO `/indicators` + `/indicators/[id]` 指标库页不动、保持可爬**(LIVE:/en/indicators 200)。N=5 易调(`ANON_PREVIEW_ROWS`)。tsc 0/build 874 页 0 新错。**注**:软门控(数据仍 client fetch,匿名可从 network 看 JSON)=拉新 nudge 非硬付费墙;硬门控(后端按登录裁剪)属 paywall 级决策,待 owner。
...
- **✅已修+LIVE验证(commit 3b0cb3d):研报资金面对大多数票静默缺 OPTIONS 块**。根因:研报 fact sheet 按 (ticker,ET-day,lang) 缓存,options 块读 `OptionsCache.Cached`(cache-only,设计上不阻塞 assemble);但 `c.cache`(每票 view)只被**按需 `Options()` 调用**(/options 命中)填充,**`scanUnusual` 每 30m 拉每个 mega-cap 的链却只建 unusual 看板、从不填 `c.cache`** → 除非某票在其研报当天首次 assemble 前 15m 内被 /options 命中,否则研报缺 options 块,且当天缓存冻结(实测 AAPL 3 次取 generated_at 相同,restart 后冷态 assemble;期间 /options 一直显示 max_pain 292.5)。非审计修的回归(cboe 正常)。**修(便宜,已做)**:抽 `viewFromChain` 纯函数(不拉链),`compute()` 委托它(行为不变);`scanUnusual` 复用其**单次**链拉取顺便算 view 写 `c.cache[tk]`(同 map/TTL,**无双倍 Cboe 拉取**)。~40 mega-cap 常温,研报可靠含 options;restart 冷窗缩到首轮扫描(~40s)。非扫描冷门票保持按需。**LIVE 验证:AAPL 研报现含 max pain $292.50 + p/c 0.63/0.71;TSLA $400;NVDA $205(均 Cboe ~15min 延迟署名)**——此前 options 块完全缺失。

## 🔬 v8 指标引擎正确性审计(2026-06-15,owner 休息中自主)
> 对喂 IndicatorsPanel + 研报技术/基本面节的 ~161 指标 compute 引擎做对抗审计(Workflow 7 族 finder[移动平均/振荡器/量能/趋势波动/基本面G0/G1-4/复合分]→独立 skeptic refute,公式 vs 标准定义)。
- **✅审计结果(commit f583b4e,Go,已部署 LIVE 验证)**:6 confirmed formula bug 全修,**2 族干净**(振荡器 + 基本面 Group-0),0 refuted。修复:
  - **technical**:KAMA(原在末尾 reseed 原始价+只迭代 period 步,丢~250 bar IIR 递归,ranging~2.4%错)→ seed once+全 history;KVO(原 signed raw volume≈OBV,丢 Volume Force 量级)→ 标准 VF=`vol·|2(dm/cm)−1|·trend·100`+趋势翻转重置 cm;Parabolic SAR(原只 clamp 1 个前 bar+对未 clamp 的 SAR 测反转)→ Wilder 两前 bar bound 且先 clamp 后测。
...
## 🚀 v8 owner GO 批次(2026-06-16 owner 回来后决定 + 4问题修 + A→D→C→B)
> owner 回来后:报了 4 个问题(全修 LIVE:新IPO价兜底/movement中英/Material-Insider下移/per-IP限流)+ rate-cut 移出首页;给了决策并 GO 开发计划。
- **✅A 深度研报升级**(commit fcd004f 后端 + 3eddd3a 前端,LIVE 验证):**async 生成**(`?depth=deep` 秒返 data-only + `prose_status`[ready/generating/quota_exhausted/llm_disabled] + detached bg goroutine[`context.Background()`+60s+`cloneFactSheet` 修 in-place-prose race]+ atomic single-flight + quota-once-on-success + global-cap-refund + panic-safe);**配额日→ET月**(1 report/user/month,`DEEP_RESEARCH_MONTHLY_LIMIT`);over-quota=优雅 200 data-only 非 429;前端 `DeepResearchView` ~4s 轮询(cap~25/100s)+ **向后兼容**(prose_status 缺失/prose present→ready 不 poll);normal /research 仍 sync 不变;**`LLM_DEEP_MODEL=deepseek/deepseek-r1`** 已设 VPS .env(更强;不设则 fallback V3 付费)。验证:部署真实、无 sync 回归(PLTR llm=true 6节)、anon deep 401、healthz ok、LLM 健康(morning briefing+PLTR)。**owner 澄清:V3-0324 是付费非免费;$5 已充。**
- **✅D 个股页分 Tab**(commit df00170,LIVE):Overview(价/基本面/K线/指标/AI总结/异动)+ Details(财报/内部人/国会/鲸鱼/期权/新闻/讨论+原 per-section tabs);header 常显;`ANCHOR_TAB`+hashchange 让研报 citation 跨 tab 可跳;both panels `hidden` 非 unmount;i18n。
- **✅C 反爬 app 层**(commit c7eae11,LIVE 验证 throttle bot):per-IP token bucket 300rpm/60burst,CF-Connecting-IP,exempt healthz+/v1/stream,fail-open。**⏳C 边缘=owner 动作**:CF 面板 WAF Rate-Limiting Rules(>300rpm/IP on /v1/*)+ Bot Fight Mode ON(边缘拦截省 VPS)。
- **⏸B Yahoo 移除**:待 owner 港股取舍(移除=腾讯/智谱/MiniMax 报价下线;paywall 推迟故荐留到 paywall 前)。on-demand quote 的 daily-candle 兜底(bars.go)不依赖 Yahoo。
- **⏸paywall**:owner 决定推迟到用户起量(没用户开浪费钱)。
