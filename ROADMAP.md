...
- 🏗 **Alerts** (greenlit, multi-tick): ✅ data layer (`store.Alert` {ticker/kind/threshold/active}
  ...
  bilingual, delivery note). Next: ⬜ evaluator goroutine (store `ListActiveAlerts`/`MarkTriggered`
  + `triggered_at` col → goroutine checks price-cross / new Form-4 / 8-K, marks triggered + surfaces
  in-app) → ⬜ web-push (self-gen VAPID + service worker).
