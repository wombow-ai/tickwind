...
- 🏗 **Alerts** (greenlit, multi-tick): ✅ data layer (`store.Alert` {ticker/kind/threshold/active}
  ...
  `MarkAlertTriggered`, deployed). Next: ⬜ evaluator goroutine (internal/ingest, ~2min:
  price-cross via quotes → MarkTriggered; pct_move/new_filing next) + `triggered_at` badge in
  API/UI → ⬜ web-push (self-gen VAPID + service worker).
