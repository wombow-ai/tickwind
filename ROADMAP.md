...
- 🏗 **Alerts** (greenlit, multi-tick): ✅ data layer (`store.Alert` {ticker/kind/threshold/active}
  ...
  + in-app triggered badge; live, tested). Next (optional): ⬜ ⑤ web-push delivery — needs a new dep
  (webpush-go) + VAPID + service worker + subscribe API; works on desktop/Android, **iOS needs PWA**;
  email is an alt (needs SMTP creds). **Owner decision: build web-push, or pause Alerts here?**
