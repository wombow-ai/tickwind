...
- **✅进度 (2026-06-17,owner 回来后)**:**B 已做(owner 选"现移除")**——Yahoo 移除核心 LIVE(782163d:指数→Alpaca ETF 代理 SPY/DIA/QQQ、US 价正常、HK 0700/2513/0100+恒生下线、F&G 重加权);残留 quote-guard(bcfb6f8)**已 LIVE**使 HK 不再显示残留 Yahoo 旧价(0700.HK 现 404 "no quote yet"→"—",AAPL 仍 alpaca $298.21)。**慢/卡诊断完成**:根因=LLM(V3/OpenRouter)那时~12s 上游延迟→AI 接口(/summary、/movement、/research)各~12s 卡,服务器本身健康(非爬虫/非 OOM)。**荐修=把 /summary+/movement 异步化(同深度研报 A)**,等你「做」。**C**:你已配 CF 面板边缘规则=每 IP 每 10s 50 请求(=300/min,面板只到 10s 粒度,与 app 层 300rpm 一致)。
- **✅进度 (2026-06-17,owner「继续开发」→ 慢/卡根治 LIVE)**:把 /summary + /movement **异步化已做并 LIVE 验证**(commit 96c149b)——两接口现秒返(数据/canned line + prose_status),LLM 移到 detached 后台 goroutine,前端轮询(同深度研报模式)。**公网实测:/summary AAPL 1.6s(ready 1265 字)、NVDA/SOFI 1.2-1.5s(冷→秒返 generating,后台生成完再轮询变 ready 带真摘要 1233/1412 字);/movement 0.97-1.4s——原 ~12s 阻塞 → 现 ~1-1.6s,慢的根因彻底解决**。至此你报的两个问题(慢 + 英文页中文混用)都根治 LIVE。**剩 owner-gated/blocked DEFERRED**:HK 财报、KR 市场、web-push、paywall(用户起量后)、dual-class 正确总市值(需 raw-XBRL 管线,见 #6)。
- **🟢待拍板 (2026-06-18,owner 长期不回时我自主 bounded 调研出的发现):HK 财报现已可建。** 你跟的 3 只港股(Tencent 0700 / 智谱 02513 / MiniMax 00100)移除 Yahoo 后没了行情、变薄。我做了 2 次 read-only 探测(VPS,residential proxy):**之前"HK 财报 blocked"的根因(`prefix.do` 返空)其实是个参数 bug——用 `lang=EN`(大写,不是 `lang=en`)就正常返 stockId(Tencent 0700→7609)**;`titleSearchServlet`(按 stockId 拉公告)+ 静态 `ListOfSecurities.xlsx`(1.4MB 可下)都通。**结论:HK 财报-only 可建**(小 `internal/hkex` client + `HKAdapter` filings-only,像 TWAdapter;免费、可转售-clean;详见 `docs/hk-kr-watchlist-feasibility.md` 2026-06-18 段)。**这是 deferred 路线图项 + 产品决策,我没擅自建——你说一句「做 HK 财报」我就开**(委派 opus + 我中心化提交/gate/部署/公网验)。不想做就继续 DEFERRED,你的 3 只港股保持"代码登记但无数据"。
- **✅进度 (2026-06-17,owner 3 新需求全部 LIVE 验证)**:**①个股页扁平化 tab**(331d5d4 web)——去掉 Details 子 tab,顶级 tab 现为 Overview/Research/Filings & Money/News & Discussion/My(登录);citation 跨 tab 跳转保留(ANCHOR_TAB+hashchange 自动切 tab)。**②Deep Research 导出 PDF**(331d5d4 web)——无依赖 window.print()+scoped print CSS,只打印研报本身;**图片导出跳过**(要重型 html-to-canvas 依赖,与"简单的话可以做,不简单就算了"不符)。**③英文页中文泄漏清扫**(b7727bf Go)——你报的 "Short Trend=上升/rising" + "F&G=56(贪婪)" 根因=多个 Go 拥有的 fact 值硬编码中文不分语言;加 pickLang 后改为语言感知:shortTrend、congress 买卖方向、13F 标签、F&G 标签(现"42 (Fear)")、社媒前值注、亏损 P/E、3 个 source 常量、3 处 disclaimer(research/movement/materialevents)。EN 报告现**任何 fact 值零 CJK**(已 live curl 断言)。防幻觉契约不变(Go 仍拥有每个数字,只是标签字符串变语言感知)。**仅 ②图片导出主动跳过**——若你想要图片导出我可加(需引入依赖),否则 PDF/打印已够。
